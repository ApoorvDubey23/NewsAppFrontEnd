...
function Box({title,description,urlToImage,url,author,publishedAt,mode}) {
  const desc=description && description.slice(0,150)+"....";
  return (

    <Card className={ `${mode?" bg-gray-200 text-black":"bg-black text-white"}`} style={{height:"100%", width: '17rem',display:"flex" }}>
      <Card.Img style={{height:'180px'}} variant="top" src={urlToImage} />
      <Card.Body className={`${mode?" bg-gray-200 text-black":"bg-black text-white"}`} style={{   display: "flex",flexDirection: "column",
